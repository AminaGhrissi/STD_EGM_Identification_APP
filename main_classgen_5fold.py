...
#################################################################################
## Main script
# The following folders should contain data (original or transformed):
# Bin_EGM_plots1 , labels , EGM_plots_3slices , data
#################################################################################
...
from data_io import labels_of_files, adjust_batches_egms, adjust_batches_egms_SVM, DataGenerator, data, data_generator, get_f_names, balance_oversampling_files
from train_graph import STD_classifier
from evaluate_model import plots, eval_and_store, eval_and_store_svm
...
test_NPV = [None] * k
test_F1 = [None] * k
r = [None] * k ## r =  number of principal components (PC) in PCA
...
for train_val, test in kfold.split(f_names, y_f_names):
    ...
    if parameters["architecture"]=="pca_svm":

        ## Partition datasets
        ##  Adjust mini batches so that all batches contain the same number of samples
        print('adjust train')
        train_files, y_train, x_train = adjust_batches_egms_SVM(train_files, parameters["batch_size"], parameters["VAVp"], parameters["image"], parameters["image3D"], parameters["tensorize"])
        print('adjust val')
        val_files, y_val, x_val = adjust_batches_egms_SVM(val_files, parameters["batch_size"], parameters["VAVp"], parameters["image"], parameters["image3D"], parameters["tensorize"])
        print('adjust test')
        test_files, y_test, x_test = adjust_batches_egms_SVM(test_files, parameters["batch_size"], parameters["VAVp"], parameters["image"], parameters["image3D"], parameters["tensorize"])
