...
def read_pt_saveas_img_BIN(pt_name, max_lead, slice):
    ...
    img_path = os.path.join('DL_STD_classif','EGM_plots'+str(slice+1))
    ...
    Bin_img_path = os.path.join('DL_STD_classif','Bin_EGM_plots'+str(slice+1))
...
img3D_path = img_path = os.path.join('DL_STD_classif','EGM_plots3D')
...
for pt_name in f_names:
    ...
    img_path1 = os.path.join('DL_STD_classif','Bin_EGM_plots1')
    ...
    label_path = os.path.join('DL_STD_classif','data')
    with open(os.path.join(label_path,pt_name), 'rb') as ff:
        egm_matrix = pl.load(ff)
        lab = pl.load(ff)

    if not os.path.exists(os.path.join('DL_STD_classif','EGM_plots_3slices')): # Create target Directory if doesn't exist
        os.mkdir(os.path.join('DL_STD_classif','EGM_plots_3slices'))

    # PIK = os.path.join('DL_STD_classif','EGM_plots_3slices', pt_name.split(".")[0]+'.dat')
    # with open(PIK, 'wb') as f:
    #     pl.dump(egm_pat_pt_load_slice, f) 
    #     pl.dump(lab, f)  
    
    PIK = os.path.join('DL_STD_classif','EGM_plots_3slices', pt_name.split(".")[0]+'.npy')
